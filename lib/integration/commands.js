export function registerWikiCommands(ctx, wikiRoot, scanner, indexer, graph, proposalStore) {
    ctx.inject(['commands'], (commandCtx) => {
        commandCtx.commands.register({
            name: 'wiki',
            description: '个人长期 Wiki 上下文与待审提案管理',
            input: {
                hint: 'status | list | diff <id> | approve <id|all> | reject <id|all> | reindex',
            },
            handler: async (_session, input) => {
                ...
                switch (action) {
                    case 'status': {
                        ...
                        return {
                            ...
                            text: [
                                ...
                                '',
                                '常用指令: `/wiki list` (查提案) | `/wiki approve <id|all>` (批准写入) | `/wiki reindex` (重构索引)',
                            ].join('\n'),
                        };
                    }
                    case 'list': {
                    ...
                    case 'approve': {
                        if (!arg) {
                            return { kind: 'error', text: '请提供待批准的提案 ID，例如: `/wiki approve prop-1234`，或 `/wiki approve all` 批准全部' };
                        }
                        if (arg.toLowerCase() === 'all') {
                            const pending = proposalStore.listPending();
                            if (pending.length === 0) {
                                return { kind: 'success', text: '当前没有待处理的提案。' };
                            }
                            const results = [];
                            for (const p of pending) {
                                const res = await proposalStore.approve(p.id);
                                results.push(`- [${p.id}] ${res.ok ? '✅' : '❌'} ${res.message}`);
                            }
                    ...
                    case 'reject': {
                        if (!arg) {
                            return { kind: 'error', text: '请提供要废弃的提案 ID，例如: `/wiki reject prop-1234`，或 `/wiki reject all` 废弃全部' };
                        }
                        if (arg.toLowerCase() === 'all') {
                            const pending = proposalStore.listPending();
                            if (pending.length === 0) {
                            ...
                            for (const p of pending) {
                                await proposalStore.reject(p.id);
                            }
                            return { kind: 'success', text: `🗑️ 已批量废弃全部 ${pending.length} 条提案。` };
                        }
